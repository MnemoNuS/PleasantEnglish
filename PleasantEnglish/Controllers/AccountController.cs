...
{
	[Authorize]
	public class AccountController : BaseBlogController
	{
		...
		{
			PrepareData();
			ViewBag.ReCaptchaKey = WebConfigurationManager.AppSettings["reCAPTCHAKey"];
			...
			if (IsCaptchaValid)
			{
				...
				if (ModelState.IsValid)
				{
					...
					if (result.Succeeded)
					{
						...
						MailHelper.RegistrationMail(user);

						return RedirectToAction("Index", "Home");
					}
					if( db.Users.Any( u => u.Email == model.Email ) )
					{
						ModelState.AddModelError( "", @"Email уже используется на нашем сайте" );
					}
					else
					{
						AddErrors( result );
		...
		public async Task<ActionResult> ForgotPassword(ForgotPasswordViewModel model)
		{
			...
			if (ModelState.IsValid)
			{
				...
				if( user == null || !( await UserManager.IsEmailConfirmedAsync( user.Id ) ) )
				{
					...
					MailHelper.ForgotPassword( user, callbackUrl );
