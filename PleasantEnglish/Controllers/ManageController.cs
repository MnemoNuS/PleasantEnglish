...
{
    [Authorize]
    public class ManageController : BaseBlogController
	{
		...
		{
			PrepareData();
			var user = AuthHelper.GetUserFromCookie();
			if ( user != null )
			{
				...
				if( dbUser != null )
				{
					...
					AuthHelper.SetUserCookie(dbUser);
		...
		//
		public async Task<ActionResult> ChangeImage(ChangeImageViewModel model)
		{
			PrepareData();
			var user = AuthHelper.GetUserFromCookie();
			if( user != null )
